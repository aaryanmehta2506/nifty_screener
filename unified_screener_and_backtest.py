...
CONFIG = {
    ...
    # Output
    "csv_output_swing": os.path.join(_THIS_DIR, "swing_book_3_6mo.csv"),
    "csv_output_core": os.path.join(_THIS_DIR, "core_book_1_2yr.csv"),
    "csv_output_crossover": os.path.join(_THIS_DIR, "crossover_highest_conviction.csv"),
    "csv_output_bulkdeals": os.path.join(_THIS_DIR, "bulk_deals_matched.csv"),
    "csv_output_portfolio": os.path.join(_THIS_DIR, "final_portfolio_selection.csv"),
}
...
# FIX v7.2: yfinance has its own internal logger that prints things like
# "AWL.NS: No earnings dates found, symbol may be delisted" directly to
# the console whenever Yahoo's data coverage has a gap for a ticker —
# this is NOT an error surfaced by this script. check_earnings_blackout()
# already catches the resulting exception/empty-result internally and
# correctly falls through to "no data, not blocking" — the trade
# evaluation is unaffected either way. This was just visual noise from a
# library logger we don't control the wording of. Raised to CRITICAL so
# it stops cluttering real runs; this script's OWN log messages (the
# "screener" logger above) are untouched and still print normally.
...
def fetch_fundamentals(symbol: str) -> Fundamentals:
    ...
    def safe_div_yield(key):
        # FIX v7.4 — yfinance has inconsistently changed whether
        # dividendYield is returned as a fraction (0.0226 = 2.26%, the
        # older/expected format) or already-as-percentage (2.26 meaning
        # 2.26% directly) depending on version/ticker. Blindly multiplying
        # by 100 either way produced impossible values like "226.0%" for
        # LICI, "148.0%" for BAJAJ-AUTO, etc — a real dividend yield can
        # never exceed 100% (a fraction of 1.0), so any raw value > 1
        # can ONLY be the already-a-percentage format, and is used as-is.
        v = info.get(key)
    ...
    result = Fundamentals(
        ...
        debt_eq=safe_num("debtToEquity", 3), div_yield=safe_div_yield("dividendYield"),
        beta=safe_num("beta"),
...
_SECTOR_GROUPS = {
    ...
    "Metals & Mining": ("JSWSTEEL", "TATASTEEL", "HINDALCO", "HINDZINC", "SAIL", "NMDC", "VEDL", "NLCINDIA",
                         ...
                         "MMTC", "LLOYDSME", "HEG", "GRAPHITE", "GRAVITA", "HINDCOPPER", "GPIL", "SHYAMMETL"),
    ...
    "Consumer/Retail": ("TITAN", "TRENT", "DMART", "NYKAA", "ETERNAL", "SWIGGY", "MARICO", "NESTLEIND",
                         ...
                         "KALYANKJIL", "LTFOODS", "JUBLFOOD", "BIKAJI", "CCL", "DOMS", "HONASA"),
    ...
    "Auto & Ancillaries": ("MARUTI", "TATAMOTORS", "EICHERMOT", "HEROMOTOCO", "BAJAJ-AUTO", "MOTHERSON",
                            ...
                            "JKTYRE", "CEATLTD", "ENDURANCE", "SCHAEFFLER", "TIMKEN", "GABRIEL",
                            "JBMA", "MSUMI", "OLECTRA", "HYUNDAI", "ASAHIINDIA"),
    ...
    "Real Estate": ("DLF", "GODREJPROP", "OBEROIRLTY", "PRESTIGE", "PHOENIXLTD", "LODHA", "BRIGADE", "SOBHA",
                     ...
                     "SUNTECK", "MAHLIFE"),
    "Diversified Financials": ("BAJFINANCE", "BAJAJFINSV", "CHOLAFIN", "ABCAPITAL", "JIOFIN",
    ...
    "Telecom": ("BHARTIARTL", "IDEA", "TATACOMM", "INDUSTOWER", "HFCL", "BHARTIHEXA"),
    # NEW v7.4 — added after a real run showed KALYANKJIL (+10% the next
    # day) getting rejected from the final portfolio purely because
    # MMTC/MCX/LLOYDSME had already filled "Diversified/Other"'s 3 slots
    # first, despite having nothing in common as businesses. These new
    # categories give genuinely comparable stocks their own competition,
    # instead of unrelated businesses fighting over the same slots.
    "Capital Markets Infra": ("BSE", "CDSL", "MCX", "CAMS", "KFINTECH", "IEX",
                                "ANGELONE", "MOTILALOFS", "IIFL", "JMFINANCIL", "ANANDRATHI"),
    "Business/Support Services": ("ECLERX", "BLS", "TATATECH", "LATENTVIEW", "INTELLECT"),
    "Housing Finance/NBFC": ("M&MFIN", "LICHSGFIN", "PNBHOUSING", "AAVAS", "HOMEFIRST",
                              "CANFINHOME", "AADHARHFC", "SBFC", "CREDITACC", "FIVESTAR",
                              "BAJAJHFL", "SHRIRAMFIN", "SUNDARMFIN", "MFSL"),
...
def print_report(swing_df, core_df, crossover_df, bulk_deals_df, macro_events, market_regime=None,
    ...
    print(f"\n{'─'*95}\n  📋 REJECTION BREAKDOWN BY CATEGORY (NEW — answers 'why did so few pass?')\n{'─'*95}")
    if not REJECTION_LOG:
        print("  Nothing rejected this run.")
    else:
        categories = {}
        for sym, reason in REJECTION_LOG.items():
            # Bucket by everything before the first "(" or em-dash — keeps
            # useful distinctions like "Swing: failed trend template" vs
            # "Swing: VCP invalid" as separate categories, not both
            # collapsed into a single unhelpful "Swing" bucket. Also strips
            # specific numbers so "Insufficient data: 233 bars" and
            # "...244 bars" collapse into one category instead of each
            # exact bar count fragmenting the summary.
            key = re.split(r"[(—]", reason)[0].strip()
            key = re.sub(r"\d+", "N", key)
            categories[key] = categories.get(key, 0) + 1
        for cat, count in sorted(categories.items(), key=lambda kv: -kv[1]):
            print(f"  {count:>4}  {cat}")
        print(f"\n  Total rejected: {len(REJECTION_LOG)}")
